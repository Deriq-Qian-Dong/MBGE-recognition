...
def dump_frames(vid_path):
    ...
    out_full_path = os.path.join(out_path, vid_name[-4],vid_name[-3],vid_name[-2],vid_name[-1].split('.')[0])
...
def run_optical_flow(vid_item, dev_id=1):
    vid_path = vid_item[0]
    vid_id = vid_item[1]
    vid_name = vid_path.split('/')
    out_full_path = os.path.join(out_path, vid_name[-4],vid_name[-3],vid_name[-2],vid_name[-1].split('.')[0])
    ...
    os.system(cmd)
    print '{} {} done'.format(vid_id, vid_name[-1].split('.')[0])
    sys.stdout.flush()
    return True
...
def run_warp_optical_flow(vid_item, dev_id=0):
    ...
    os.system(cmd)
    print 'warp on {} {} done'.format(vid_id, vid_name[-1].split('.')[0])
    sys.stdout.flush()
    return True
